...
class TestPdep(unittest.TestCase):
    ...
    def test_pdep_a(self):
        ...
        """
        pdep_city = round(pdep.pdep(self.df, self.counts_dict, tuple([self.city])), 2)
    ...
    def test_pdep_a_b(self):
        ...
        """
        pdep_zip_city = round(
            pdep.pdep(self.df, self.counts_dict, self.zip_code, self.city), 2
        )
    ...
    def test_expected_pdep_a_b(self):
        ...
        epdep_zip_city = round(
            pdep.expected_pdep(self.df, self.counts_dict, self.zip_code, self.city), 2
        )
...
class TestErrorPdep(unittest.TestCase):
    ...
    def test_pdep_a_error(self):
        ...
        """
        error_dict = {(0,0): '0'}
        counts_dict = pdep.calculate_counts_dict(self.df, error_dict, order=1)
        pdep_id = round(pdep.pdep(self.df, counts_dict, tuple([0])), 2)
    ...
    def test_pdep_a_all_error(self):
        ...
        marked as an error.
        """
        error_dict = {(0,0): '0', (1,0): '1', (2,0): '2', (3,0): '3'}
        counts_dict = pdep.calculate_counts_dict(self.df, error_dict, order=1)
        pdep_id = round(pdep.pdep(self.df, counts_dict, tuple([0])), 2)
    ...
    def test_pdep_a_b_error_lhs(self):
        ...
        """
        error_dict = {(0,0): '0', (1,0): '1'}
        counts_dict = pdep.calculate_counts_dict(self.df, error_dict, order=1)
        pdep_a_b = round(pdep.pdep(self.df, counts_dict, tuple([0]), 1), 2)
    ...
    def test_pdep_a_b_all_error_rhs(self):
        ...
        Test that pdep(id, name) is None if all values in A are errors.
        """
        error_dict = {(0,0): 'Otto', (1,0): 'Hanna', (2,0): 'Peter', (3,0): 'Ella'}
        counts_dict = pdep.calculate_counts_dict(self.df, error_dict, order=1)
        pdep_a_b = round(pdep.pdep(self.df, counts_dict, tuple([0]), 1), 2)
        self.assertIsNone(pdep_a_b)
