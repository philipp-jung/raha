...
def pdep_vicinity_based_corrector(
    ...
    Return a list of lists of dictionaries. Each of the N lists in the list represents the features generated by one of
    the top N FDs. Each type of Feature is a dictionary in that nested list, so there are up to 4 elements in the
    returned list. Each dictionary contains key-value-pairs of 'correction suggestion': float.
    """
    ...
    for lhs_cols, pdep_tuple in gpdeps_subset.items():
        ...
        if rhs_col not in lhs_cols and lhs_vals in counts_dict[lhs_cols][rhs_col]:
            ...
            for rhs_val in counts_dict[lhs_cols][rhs_col][lhs_vals]:
                pr = counts_dict[lhs_cols][rhs_col][lhs_vals][rhs_val] / sum_scores
                correction_suggestion = rhs_val

                prs[correction_suggestion] = pr
